...
func TestConfigHelpers(t *testing.T) {
	...
	// Test Collections helpers
	cfg.AddToCollection("Coding", "m1")
	cfg.AddToCollection("Coding", "m2")
	if len(cfg.Collections["Coding"]) != 2 {
		t.Errorf("expected 2 models in Coding collection, got %d", len(cfg.Collections["Coding"]))
	}
	cfg.AddToCollection("Coding", "m2") // Duplicate add
	if len(cfg.Collections["Coding"]) != 2 {
		t.Errorf("expected duplicate add to be ignored, got count %d", len(cfg.Collections["Coding"]))
	}
	cfg.RemoveFromCollection("Coding", "m1")
	if len(cfg.Collections["Coding"]) != 1 || cfg.Collections["Coding"][0] != "m2" {
		t.Errorf("expected only 'm2' in Coding, got %v", cfg.Collections["Coding"])
	}
	cfg.RemoveFromCollection("Coding", "m2")
	if _, ok := cfg.Collections["Coding"]; ok {
